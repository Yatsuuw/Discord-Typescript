...
import { EmbedBuilder, CategoryChannel, PermissionsBitField, TextChannel, ActionRowBuilder, ButtonBuilder, ButtonStyle, GuildMemberRoleManager } from 'discord.js';
            ...
            db.get('SELECT logChannelId FROM servers_settings WHERE guildId = ?', [guild.id], async (err, rows: ServerSettings) => {
                ...
                if (logChannelId) {
                    try {
                        ...
                        if (!ticketsModId) {
                            console.error('Error : The moderation role ID for tickets is not defined.');
                            return;
                        }
                        ...
                        console.error(`Error retrieving the log room for server ${guild.name} (${guild.id}). Error : `, error);
                    }
                }
            })
        });
